...
#define LV_MEM_SIZE (200U * 1024U)  /* 200KB — bumped from 48KB: all screens
                                        (status bar, 4 menus, 2 list pages,
                                        confirm screen) are built once at
                                        boot and stay alive simultaneously,
                                        so the pool has to hold every
                                        object/label/style at once, not
                                        just the currently visible screen. */
#define LV_MEM_ADR 0
...
// Both were 1/80 by default: the theme "grows" buttons on press and
// fades style changes over 80ms. Each of those is another partial
// redraw with no vsync sync on this panel (see main.cpp's flush
// comment) — every single tap was re-triggering the tearing/glitching
// we've been chasing. Off entirely, on request: hard/instant over smooth.
